...
import { Vector, distance2D, normalize2D, type IPoint2D, type IVector2D } from '../../sim/Geometry';
...
export class StraightRoadPrimitive extends RoadPrimitive {
    ...
    private static extremityCutValues(cut: IExtremityCut | undefined): [number, number, number, number, number] {
    ...
    private static getSideCutSegments(params: {
        ...
        const { cuts, overlapStartX, overlapEndX } = params;
        if (!cuts?.length || overlapEndX <= overlapStartX) return [];

        const toLocalX = (value: number): number => value;
    ...
    private createCutGeometry(params: {
        ...
        const leftOuter = 0;
        const roadLeft = carriagewayStartM;
        const middle = (carriagewayStartM + carriagewayEndM) * 0.5;
        const roadRight = carriagewayEndM;
        const rightOuter = widthM;
        const lateral = [leftOuter, roadLeft, middle, roadRight, rightOuter];

        const startCuts = StraightRoadPrimitive.extremityCutValues(this.cuts?.entryCut);
        const endCuts = StraightRoadPrimitive.extremityCutValues(this.cuts?.exitCut);
        const repeat = endV - startV;

        const startX = lateral.map((_, index) => startCuts[index]);
        const endX = lateral.map((_, index) => length - endCuts[index]);
        ...
        for (const point of contour) {
            const t = THREE.MathUtils.clamp(point.x / length, 0, 1);
            const u = THREE.MathUtils.clamp(point.y / widthM, 0, 1);
    ...
    override createGeometry(params: {
        ...
        const latZ = dirX;
        const baseX = this.entry.x - latX * widthM;
        const baseZ = this.entry.z - latZ * widthM;
        ...
        if (!hasCustomCuts) {
            // Generate 4 corners directly in world XZ from entry outer-left origin
            const positions: number[] = [];
            const pushWorld = (along: number, lat: number) =>
                positions.push(baseX + dirX * along + latX * lat, y, baseZ + dirZ * along + latZ * lat);
            pushWorld(0, 0);  // 0: entry, outer-left
            pushWorld(0, widthM);  // 1: entry, outer-right
            pushWorld(length, 0);  // 2: exit,  outer-left
            pushWorld(length, widthM);  // 3: exit,  outer-right
            const geometry = new THREE.BufferGeometry();
        ...
        return this.createCutGeometry({
            ...
            startV, endV,
            dirX, dirZ, latX, latZ, midX: baseX, midZ: baseZ, y,
        });
    }
    ...
    override getGeometry(line: RoadLine) {
        ...
        const offset = this.getLineLateralOffset(line) - halfWidth;

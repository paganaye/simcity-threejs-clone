...
export class Population {
    ...
    init(mapWidth: number, mapHeight: number, options: PopulationOptions = {}): void {
        ...
        for (let i = 0; i < targetCrowd; i++) {
            ...
            const scale = isChild ? appConstants.ChildHeightInMetre / appConstants.CharacterHeightInMetre : 1;

            const speedMetersPerSecond = isChild
                ? 1.0 + Math.random() * 0.5
                : 1.2 + Math.random() * 0.6;
            const speed = walking ? speedMetersPerSecond * worldUnitsPerMeter : 0;

            this.crowdAgents.push({ x, z, heading, speed, walking, scale });
